		...
		if(tempTime > nextDPTS):
			...
			if(convert_to_seconds(timeDiff) > ((CONSECUTIVE_DP_TIME_DIFF-CONSECUTIVE_DP_TIME_DIFF_THRES)*timeUtils.NUM_SEC_IN_MIN)):
				...
				if(len(rowsDF) >= 18):
					...
					rowsPDDF = pd.DataFrame(rowsDF, columns =['DateTime', 'LAT', 'LON', 'SOG', 'COG'])
...
def main():
	...
	srcDir = (config['INTERP_TRAJ']['SRC_DIR'])
	srcTrainFile = (config['INTERP_TRAJ']['SRC_TRAIN_LIST'])
	destDir = (config['INTERP_TRAJ']['DEST_DIR'])
